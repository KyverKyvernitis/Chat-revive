# Core Worker 0.7.5 — launcher Gradle compatível com Android

A versão `0.7.5` usa o Termux somente como **builder bootstrap**, mantendo Termux e APK como runtimes separados durante a transição. O primeiro APK dessa transição é compilado e assinado pelo `phone_worker.py` no celular, com a mesma keystore das versões já instaladas. A VPS não executa Gradle, Android SDK, NDK ou `aapt2`: ela prepara o ZIP de fontes, entrega segredos temporários por job autenticado e publica o APK pronto recebido do celular.

O bundle do toolchain agora usa manifesto v5. Além de `executablePaths`, ele normaliza o `DEFAULT_JVM_OPTS` do launcher Gradle para o `/system/bin/sh`, impedindo que o Java interprete `"-Xmx64m"` como classe principal. O APK rejeita bundles antigos sem essa garantia. A publicação lê `packageName`, `versionName` e `versionCode` diretamente do `AndroidManifest.xml` compilado; metadados divergentes ou downgrade preservam o APK e o `latest.json` anteriores.
...
O painel do Discord agora mostra um único estado por pipeline, diferencia fila ativa de falha encerrada e exibe o detalhe do último erro sem repetir duas mensagens de APK. O aplicativo concentra conexão, runtime e autobuild no primeiro card, mantém diagnóstico e manutenção recolhidos e apresenta o preflight do builder com estados visuais distintos: verificando, pronto, republicação disponível ou bloqueado.

A automação reconcilia `core_worker_automation_pending.json` com o estado real do job no registry. Um job `failed`, `expired` ou `cancelled` deixa de aparecer como “build pendente”; retry automático respeita cooldown e retry manual continua disponível após a correção.
...
```text
Primeira transição
VPS prepara fonte/segredos → Termux gera o bundle mínimo e compila/assina → VPS valida a identidade compilada e publica → APK 0.7.5 é instalado
...
O manifesto usa o schema `core-worker-android-builder-v1`, versão 5, arquitetura ARM64, runtime `termux-bionic-direct`, registra a estratégia `dt-needed-transitive-v1` e o launcher `android-sh-unquoted-default-jvm-opts-v1`. O Gradle valida o ZIP antes de empacotá-lo. O APK executa uma validação mais forte após extrair o bundle e não depende do rootfs de Bedrock, PRoot ou de Python externo para buildar.
...
> As seções abaixo registram patches antigos e podem citar fluxos anteriores. O comportamento atual é o descrito nesta seção 0.7.5.
