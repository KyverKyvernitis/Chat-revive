...
class TtsVoiceCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # anti-duplicação (evita responder duas vezes pra mesma msg)
    ...
    async def _reply_temp_error(self, message: discord.Message, content: str, delay: int = 7):
        """Responde e apaga a msg do usuário + resposta do bot após Xs."""
        try:
    ...
    async def _synthesize_gtts(self, text: str, out_path: str):
        # gTTS não tem pitch/rate/voz real; só gera pt-br simples
    ...
    async def _speak_from_message(self, message: discord.Message, text: str):
        ...
        # bloqueia se existir bot informado
        ...
        # resolve config (user -> server -> fallback)
        ...
        async with lock:
            ...
            try:
                ...
                await self._reply_temp_error(message, f"❌ Falha no TTS ({engine}): `{type(e).__name__}` — `{e}`")
            finally:
                if tmp:
                    try:
                        os.remove(tmp)
    ...
    # ------ engine (user/server) ------
    @app_commands.command(name="set_tts_engine", description="Define seu motor de TTS (edge ou gtts).")
    @app_commands.describe(engine="edge (recomendado) ou gtts")
    async def set_tts_engine(self, interaction: discord.Interaction, engine: str):
        ...
        if engine not in ("edge", "gtts"):
            return await self._reply(interaction, "⚠️ Use `edge` ou `gtts`.", ephemeral=True)

        await self.bot.settings_db.set_user_tts(interaction.guild.id, interaction.user.id, engine=engine)

        if engine == "gtts":
            await self._reply(interaction, "✅ Motor definido para **gTTS**. (voz/speed/tone não têm efeito nele)", ephemeral=True)
        else:
            await self._reply(interaction, "✅ Motor definido para **edge-tts**.", ephemeral=True)

    @app_commands.command(name="set_server_tts_engine", description="Define o motor padrão do servidor (requer Expulsar membros).")
    @app_commands.describe(engine="edge (recomendado) ou gtts")
    async def set_server_tts_engine(self, interaction: discord.Interaction, engine: str):
        ...
        await self.bot.settings_db.set_guild_tts_defaults(interaction.guild.id, engine=engine)

        if engine == "gtts":
            await self._reply(interaction, "✅ Motor padrão do servidor: **gTTS**. (voz/speed/tone não têm efeito nele)", ephemeral=True)
        else:
            await self._reply(interaction, "✅ Motor padrão do servidor: **edge-tts**.", ephemeral=True)

    # ------ voice/speed/tone (edge) ------
    @app_commands.command(name="set_voice", description="Define sua voz (edge-tts).")
    ...
    async def set_voice(self, interaction: discord.Interaction, voice: str):
        ...
        await self._reply(interaction, "✅ Sua voz foi atualizada (edge-tts).", ephemeral=True)

    @app_commands.command(name="set_speed", description="Define sua velocidade (edge-tts) (ex: +10%, -10%).")
    ...
    async def set_speed(self, interaction: discord.Interaction, speed: str):
        ...
        await self._reply(interaction, "✅ Sua velocidade foi atualizada (edge-tts).", ephemeral=True)

    @app_commands.command(name="set_voice_tone", description="Define seu tom (edge-tts) (ex: +50Hz, -50Hz).")
    ...
    async def set_voice_tone(self, interaction: discord.Interaction, tone: str):
        ...
        await self._reply(interaction, "✅ Seu tom foi atualizado (edge-tts).", ephemeral=True)

    @app_commands.command(name="set_server_voice", description="Define a voz padrão do servidor (edge-tts) (requer Expulsar membros).")
    @app_commands.describe(voice="Nome da voz (use /voices)")
    @app_commands.autocomplete(voice=voice_autocomplete)
    async def set_server_voice(self, interaction: discord.Interaction, voice: str):
        ...
        await self._reply(interaction, "✅ Voz padrão do servidor atualizada (edge-tts).", ephemeral=True)

    @app_commands.command(name="set_server_speed", description="Define a velocidade padrão do servidor (edge-tts) (requer Expulsar membros).")
    @app_commands.describe(speed="Ex: +10%, -10%, +0%")
    async def set_server_speed(self, interaction: discord.Interaction, speed: str):
        ...
        await self._reply(interaction, "✅ Velocidade padrão do servidor atualizada (edge-tts).", ephemeral=True)

    @app_commands.command(name="set_server_voice_tone", description="Define o tom padrão do servidor (edge-tts) (requer Expulsar membros).")
    @app_commands.describe(tone="Ex: +50Hz, -50Hz, +0Hz")
    async def set_server_voice_tone(self, interaction: discord.Interaction, tone: str):
        ...
        await self._reply(interaction, "✅ Tom padrão do servidor atualizado (edge-tts).", ephemeral=True)
