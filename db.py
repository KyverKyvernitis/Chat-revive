...
class SettingsDB:
    ...
    async def _ensure_indexes(self):
        try:
            ...
            await self.coll.create_index([("guild_id", 1), ("type", 1)], unique=True, name="guild_id_1_type_1")
    ...
    async def set_guild_tts_defaults(self, guild_id: int, *, engine: Optional[str] = None, voice: Optional[str] = None, language: Optional[str] = None, rate: Optional[str] = None, pitch: Optional[str] = None, block_voice_bot: Optional[bool] = None, only_target_user: Optional[bool] = None):
        ...
        if voice is not None: tts["voice"] = voice
    ...
    def get_user_tts(self, guild_id: int, user_id: int) -> Dict[str, str]:
        ...
        tts = u.get("tts", {}) or {}
        return {"engine": str(tts.get("engine", "") or ""), "voice": str(tts.get("voice", "") or ""), "language": str(tts.get("language", "") or ""), "rate": str(tts.get("rate", "") or ""), "pitch": str(tts.get("pitch", "") or "")}
    ...
    def resolve_tts(self, guild_id: int, user_id: int) -> Dict[str, str]:
        user = self.get_user_tts(guild_id, user_id); guild = self.get_guild_tts_defaults(guild_id)
    ...
    async def set_role_cooldown(self, guild_id: int, *, active: bool, ends_at: Optional[str] = None, role_id: Optional[int] = None, role_was_mentionable: Optional[bool] = None):
        doc = self._get_guild_doc(guild_id); cooldown = doc.get("role_cooldown", {}) or {}
        cooldown["active"] = bool(active)
        if ends_at is not None: cooldown["ends_at"] = ends_at
        if role_id is not None: cooldown["role_id"] = int(role_id)
        if role_was_mentionable is not None: cooldown["role_was_mentionable"] = bool(role_was_mentionable)
