...
class TTSVoice(TTSAudioMixin, commands.GroupCog, group_name="tts", group_description="Comandos de texto para fala"):
    ...
    @voices.command(name="edge", description="Mostra a lista de vozes do Edge em pt/pt-BR")
    async def voices_edge(self, interaction: discord.Interaction):
        await self._defer_ephemeral(interaction)
        lines = [voice for voice in (self.edge_voice_cache or []) if voice.lower().startswith("pt-")] or ["Nenhuma voz Edge em pt/pt-BR carregada."]
        await self._send_list_embeds(interaction, title="Vozes do Edge", lines=lines, footer="Mostrando só vozes do Edge em pt/pt-BR. Use `/tts voice` ou `/tts server voice` para escolher.")
